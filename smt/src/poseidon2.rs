...
//!
//! This module provides a native (off-circuit) Poseidon2 hasher that implements
//! the [`FieldHasher`] trait, making it a drop-in replacement for the Poseidon1
//! hasher in the SMT crate.
//!
//! # Parameters
//!
...
pub const RATE: usize = 2; // absorption rate
pub const R_F: usize = 8; // full rounds
pub const R_P: usize = 56; // partial rounds
pub const ROUNDS: usize = R_F + R_P; // 64 total
...
fn poseidon2_permutation<F: PrimeField>(state: &mut [F; T], params: &Poseidon2Params<F>) {
...
impl<F: PrimeField, const L: usize> Default for Poseidon2<F, L> {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    /// Verify the FieldHasher trait implementation works correctly.
    #[test]
    fn poseidon2_field_hasher_trait() {
        let hasher = Poseidon2::<Fp, 2>::new();
        let a = Fp::from(100u64);
        let b = Fp::from(200u64);

        let result = hasher.hash([a, b]).unwrap();
        assert_ne!(result, Fp::ZERO);

        // Same result via the trait constructor
        let hasher2 = <Poseidon2<Fp, 2> as FieldHasher<Fp, 2>>::hasher();
        let result2 = hasher2.hash([a, b]).unwrap();
